...
class Friends extends Component {
  ...
  getCurrent() {
    return this.props.current.map(currFriend => {
      return (
        <ListItem
          avatar
          key={currFriend.friendUserId}
          onPress={() => {
          ...
          <Body>
            ...
            <Text note>
              Confirmed: {moment(currFriend.dateAdded).format('MMM Do YYYY')}
            </Text>
            <Text note>
              {currFriend.starred === true
                ? 'Subscribed to new event posts'
                : 'Not subscribed to new event posts'}
            </Text>
          </Body>
          <Right>
            <Button
  ...
  getRequests() {
    return this.props.requests.map((currRequest, index) => {
      return (
        <ListItem avatar key={currRequest.requestorUserId}>
          <Left>
            <UserProfilePicture
              profile={{
                userId: currRequest.requestorUserId,
                profilePic: currRequest._profilePic,
              }}
              style={styles.thumbnail}
            />
          </Left>
          <Body>
            <Text>
              {currRequest._displayName || currRequest._username || 'Jane Doe'}
            </Text>
            <Text note>
              Requested:{' '}
              {moment(currRequest.dateRequested).format('MMM Do YYYY')}
            </Text>
            <Button
              danger
              transparent
          ...
          <Right>
            <Button
              ...
              transparent
              onPress={() => {
                this.confirmFriend(currRequest.requestorUserId);
              }}>
              <Text>Confirm</Text>
            </Button>
          </Right>
  ...
  getSentRequests() {
    return this.props.sentRequests.map(currRequest => {
      return (
        <ListItem avatar key={currRequest.userId}>
          <Left>
            <UserProfilePicture
              profile={{
                userId: currRequest.userId,
                profilePic: currRequest._profilePic,
              }}
              style={styles.thumbnail}
            />
          ...
          <Body>
            ...
            <Text note>
              Sent: {moment(currRequest.dateRequested).fromNow()}
            </Text>
            <Button
              danger
              transparent
