...
export const usersReducer = createReducer(applicationState.users, {
  [actionTypes.GET_LOGGED_IN_USER_INFO](state, action) {
    ...
    const profilePic = get(
      apiResponse,
      'profile.profilePic',
      'https://flaker-images.s3.amazonaws.com/default-profile.png',
    );
    const alexaSessionTokenActive = get(
      apiResponse,
      'profile.alexaSessionTokenActive',
      false,
    );
    const notifyNewEvents = get(apiResponse, 'profile.notifyNewEvents', true);
    const notifyFriendRequests = get(
      apiResponse,
      'profile.notifyFriendRequests',
      true,
    );
    const notifyHostEventChanges = get(
      apiResponse,
      'profile.notifyHostEventChanges',
      true,
    );
    const notifyJoinedEventChanges = get(
      apiResponse,
      'profile.notifyJoinedEventChanges',
      true,
    );
    ...
    set(
      stateClone,
      'loggedIn.alexaSessionTokenActive',
    ...
    set(
      stateClone,
      'loggedIn.notifyJoinedEventChanges',
