...
class ProfilePreviewModal extends React.PureComponent {
  ...
  getFavoriteButton(friendStatus, friendUserId) {
    let isStarred = false;
    const friendIndex = findIndex(
      this.props.current,
      currentFriend => currentFriend.friendUserId === friendUserId,
    );
    if (friendIndex !== -1) {
      ...
      return (
        <Button
          ...
          {isStarred ? (
            ...
            <Icon name="star-outline" style={styles.starIconOutline} />
          )}
        </Button>
      );
    }
