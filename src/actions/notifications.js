...
export const getNotifications = (dispatchFetching = false) => (
  dispatch,
  getState,
) => {
  ...
  api
    .get(`/notifications/${sessionToken}`)
    .then(apiResponse => {
      if (
...
export const markNotificationsRead = () => (dispatch, getState) => {
  ...
  const notificationIdsUnread = get(notificationsState, 'notifications', [])
    .filter(notification => {
  ...
  api
    .put(`/notifications/${sessionToken}`, {
