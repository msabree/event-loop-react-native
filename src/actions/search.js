  ...
  return new Promise((resolve, reject) => {
    Contacts.getContactsMatchingString(query, (err, res) => {
      ...
      for (let i = 0; i < matchedContacts.length; i++) {
        ...
        const mobilePhoneNumber = find(phoneNumbers, number => {
          return number.label === 'mobile';
        });
        if (mobilePhoneNumber) {
          ...
          let pic =
            'https://flaker-images.s3.amazonaws.com/default-profile.png';
          if (
            matchedContacts[i].hasThumbnail &&
            matchedContacts[i].thumbnailPath !== ''
          ) {
...
export const search = query => async (dispatch, getState) => {
  ...
  if (Platform.OS === 'android') {
    const granted = await PermissionsAndroid.request(
      PermissionsAndroid.PERMISSIONS.READ_CONTACTS,
      {
        title: 'Contacts',
        message:
          'Event Loop would like to access your contacts to help find friends.',
      },
    );
  ...
  api
    .post(`/users/search/${sessionToken}`, {
      usernameQuery: query,
    ...
    .then(apiResponse => {
      ...
      const current = get(friendsState, 'current', []).map(
        curr => curr.friendUserId,
      );
      const sentRequests = get(friendsState, 'sentRequests', []).map(
        sent => sent.userId,
      );
